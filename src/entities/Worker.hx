...
class AntAnimation {
	...
	public static var animations:haxe.ds.Map<AntAnimations, Array<h2d.Tile>> = [AntAnimations.WALK => [], AntAnimations.WALK_WITH_RESOURCE => [],];
	...
	public static function loadAnimation() {
		...
		animations[AntAnimations.WALK_WITH_RESOURCE] = [
			for (x in 0...2) spritePreProcess(antImage, x * SPRITE_SIZE, 1 * SPRITE_SIZE, SPRITE_SIZE)
		];
...
class Worker {
	...
	public function new(x:Float, y:Float, houseInit:{x:Float, y:Float}) {
	...
	public function update(dt:Float) {
		...
		if (rotationMissing < 0.05) {
			...
			var jitterRotation = new Vector(direction.x * Math.cos(determinedJitterRotation) - direction.y * Math.sin(determinedJitterRotation),
				direction.x * Math.sin(determinedJitterRotation) + direction.y * Math.cos(determinedJitterRotation)).normalized().multiply(TURNING_SPEED);
